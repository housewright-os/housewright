...
The house board: the read-only web dashboard is now two screens, each with
an anchor. A dashboard without one decays; these borrow their staying power
from a place and a ritual.

- `/` is a kitchen kiosk for a wall tablet: clock, tonight's dinner, the
  safe-to-spend chip, family events today and tomorrow, the shopping list,
  eat-first perishables, and the house list. Family-facing, glanceable
  across a room, zero inputs; writes stay in the bot so there is exactly
  one write path. Empty sections disappear entirely.
- `/review` is the money screen for the weekly partner sit-down: the
  number, the can-we-afford-it calculator, bills, essentials, runway, and
  the card-debt trend (the previous front page, unchanged).
- New `/kiosk.json` aggregate where every lane degrades independently: a
  broken section hides, it never takes down the board. Family events are
  cached ~10 minutes so the board's refresh never hammers Google. The
  server is now threaded, and all rendered text is HTML-escaped (list
  items and events originate from family texts and email extraction).

## 0.6.0

The pantry release, honest about what passive capture can know. New
pantry lane (pantry.py, every 6h): receipt emails become order events
that auto-reset the shopping lane's vendor run clocks (no more texting
"ordered walmart"); line items are recorded only when a retailer's email
actually lists them (delivery-service receipts carry none, and the lane
says so) or via an interactive --ingest-file; dedup warnings when a
shopping-list item was bought inside its shelf-life window, plus an
add-time check API; aging perishables in the evening wrap. Bought-dates
are facts, still-have is inference, and no family member ever logs
anything manually.
