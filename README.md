...
- **House board.** A read-only web dashboard with two screens: a kitchen
  kiosk for a wall tablet (clock, tonight's dinner, family events, the
  shopping list, eat-first items, the number) and a money review screen
  for the weekly partner sit-down. No inputs; the bot stays the only
  write path.
