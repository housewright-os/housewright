...
1. Confirm the profile (INTERVIEW.md) is current: equipment, allergies,
   the kid-food floor, and the batch day drive every choice below.
2. Run playbook 1 on the grocery channel first: the baseline per-meal cost
   is the number that motivates everything else.
3. Design a batch-cook week: one prep day, 4-6 dishes sharing prep and
   equipment the household actually owns, per-serving cost computed at
   shelf prices.
4. Verify the cart per rail (playbook 2) and STAGE it: the humans check
   out, always.
5. Write the prep-day runbook so every step carries its own numbers
...
6. Never build features that depend on family members logging things
