...
def mark_ordered(vendor, when=None):
    ...
    future ETAs compute from a real date instead of guessing one."""
    os.makedirs(os.path.dirname(VENDOR_ORDERS), exist_ok=True)
    orders = _vendor_orders()
    orders[vendor] = (when or date.today()).isoformat() if hasattr(when or date.today(), "isoformat") else str(when)
    tmp = VENDOR_ORDERS + ".tmp"
    with open(tmp, "w") as f:
