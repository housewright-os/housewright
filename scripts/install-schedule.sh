...
# Pantry: passive receipt-email scan every 6 hours. Order events reset the
# shopping lane's vendor run clocks; items recorded only when emails list them.
PANTRY_PLIST="$AGENTS/com.housewright.pantry.plist"
{
  echo '<?xml version="1.0" encoding="UTF-8"?>'
  echo '<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">'
  echo '<plist version="1.0"><dict>'
  echo '  <key>Label</key><string>com.housewright.pantry</string>'
  echo '  <key>ProgramArguments</key><array>'
  echo "    <string>$PY</string>"
  echo "    <string>$ROOT/scripts/pantry.py</string>"
  echo '  </array>'
  echo "  <key>WorkingDirectory</key><string>$ROOT</string>"
  echo '  <key>EnvironmentVariables</key><dict>'
  echo '    <key>PATH</key><string>/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin</string>'
  ...
  echo "    <key>TELEGRAM_BOT_TOKEN</key><string>${TELEGRAM_BOT_TOKEN:-}</string>"
  echo '  </dict>'
  echo '  <key>StartInterval</key><integer>21600</integer>'
  echo "  <key>StandardOutPath</key><string>$LOGS/com.housewright.pantry.log</string>"
  echo "  <key>StandardErrorPath</key><string>$LOGS/com.housewright.pantry.err.log</string>"
  echo '</dict></plist>'
} > "$PANTRY_PLIST"
launchctl unload "$PANTRY_PLIST" 2>/dev/null || true
launchctl load "$PANTRY_PLIST"
echo "loaded com.housewright.pantry"
