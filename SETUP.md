...
- The money lane is bring-your-own client: point `money.client_dir` in
  `config/household.json` at a directory whose `src/` provides a
  `secure_session` context manager (see the `scripts/pull.py` docstring for
  the exact contract; the reference client wraps a Monarch session held in
  the OS keychain). Leave it empty and the lane skips cleanly: the briefs
  simply omit the money block.
