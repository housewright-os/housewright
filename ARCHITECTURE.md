...
1. **Money.** `pull.py` reads accounts and transactions from Monarch
   (read-only session). `engine.py` computes Safe-to-Spend: balance minus
   bills due before payday minus reserved essentials. `notify.py` pushes
...
(every 10 min), family-events scan (every 30 min), plus the always-on bot
and a read-only LAN dashboard (`serve.py`). On Linux, translate to
