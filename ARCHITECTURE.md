...
(every 6h), rail check (monthly on the 1st), plus the always-on bot and the read-only house board (`serve.py`,
localhost by default: a kitchen kiosk at `/`, the weekly money review
at `/review`). On Linux, translate to
