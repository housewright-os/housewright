...
7. **Rail watch.** `rail_check.py` reprices the staples basket across
...
(every 10 min), family-events scan (every 30 min), rail check (monthly on
the 1st), plus the always-on bot and a read-only dashboard (`serve.py`,
