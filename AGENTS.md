...
6. **Money lane (optional, skip freely)**
   - Bring-your-own finance client: set `money.client_dir` in
     `config/household.json` per the `scripts/pull.py` docstring contract.
     Unset, the lane skips cleanly and the briefs omit the money block.
   - Check: `python3 scripts/engine.py --refresh` either succeeds or prints
     "money lane skipped" and exits without a traceback.
